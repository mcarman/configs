...
echo -e ""
echo -e "Please choose which version of Observium you would like to install${NC}"
echo -e ""
echo "1. Observium Community Edition"
echo "2. Observium Pro/Ent Edition stable (requires account at https://www.observium.org/subs/)"
echo "3. Observium Pro/Ent Edition rolling (requires account at https://www.observium.org/subs/)"
echo "4. Install the UNIX-Agent"
echo "5. Install the SNMPD (snmpd-config will be overwritten)"
echo -n "(1-5):"
read -n 1 observ_ver
...
if [ $observ_ver = 2 ] || [ $observ_ver = 3 ]; then
   echo -e "${BOLD} Your SVN username and password is found after you login at: https://www.observium.org/subs/${NC}"
...
if [ $OS = "Ubuntu" ] && [ $VER = "16.04" ]; then
   ...
   add-apt-repository multiverse
   apt -qq update
   apt -q install libapache2-mod-php8.3 php8.3-cli php8.3-mysql php8.3-gd php8.3-bcmath php8.3-mbstring php8.3-opcache php8.3-curl php-apcu php-pear snmp fping mysql-server mysql-client rrdtool subversion whois mtr-tiny ipmitool libvirt-clients graphviz imagemagick apache2 python3-mysqldb python3-pymysql python-is-python3
   a2dismod mpm_event
   a2enmod mpm_prefork
   a2enmod php8.3
